...
mod kvstore_app_integration {
    ...
    fn happy_path() {
        ...
        let res = client
            .query(RequestQuery {
                data: "unknown".as_bytes().to_owned(),
                path: "".to_string(),
                height: 0,
                prove: false,
            })
        ...
        let res = client
            ...
            .unwrap();
        assert_eq!(res.value, "6".as_bytes().to_owned());
        let res = client
            .query(RequestQuery {
                data: "1234".as_bytes().to_owned(),
                path: "bill".to_string(),
                height: 0,
                prove: false,
            })
        ...
        assert_eq!(res.value, []);
        assert_eq!(res.log, "bill does not exist");
    }

    #[test]
    fn multi_kv_pair_happy_path() {
        let mut rng = StdRng::from_seed([0; 32]);
        let kp = Secp256k1KeyPair::generate(&mut rng);
        let mutation = Mutation {
            ns: "my_twitter".as_bytes().to_vec(),
        ...
        let res = client
            .query(RequestQuery {
                data: "k2".as_bytes().to_owned(),
                path: "".to_string(),
                height: 0,
                prove: false,
            })
            .unwrap();
        assert_eq!(res.value, "value2".as_bytes().to_owned());
        let res = client
            .query(RequestQuery {
                data: "k3".as_bytes().to_owned(),
                path: "".to_string(),
                height: 0,
                prove: false,
            })
        ...
        let res = client
            ...
            .unwrap();
        assert_eq!(res.value, "14".as_bytes().to_owned());
    }
}
